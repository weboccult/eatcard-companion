        ...
        try {
            $notification = Notification::query()->create([
                ...
                'notification'    => __companionTrans('general.new_order_notification', [
                    ...
                    'username' => $order['full_name'] ?? null,
                ]),
                'type'            => 'order',
                'additional_data' => json_encode([
                    'id'                    => $order['id'],
                    'order_id'              => $order['order_id'] ?? null,
                    'order_date'            => isset($order['order_date']) ? Carbon::parse($order['order_date'])->format('d-m-Y') : \Carbon\Carbon::now()->format('d-m-Y'),
                    ...
                    'dutch_order_status'    => __companionTrans('general.'.($order['order_status'] ?? 'received')),
                    'date'                  => $data['orderDate'] ?? null,
                    'delivery_address'      => $order['delivery_address'] ?? null,
                    'method'                => isset($order['payment_split_type']) && $order['payment_split_type'] != '' ? '' : ($order['method'] ?? null),
                    'dutch_date'            => appDutchDate($data['orderDate'] ?? \Carbon\Carbon::now()->format('Y-m-d')),
                    'is_auto_print'         => (/*$store->app_pos_print &&*/ $store->is_auto_print_takeaway),
                    'is_paylater_order'     => $order['is_paylater_order'] ?? null,
                    'is_notification'       => $data['is_notification'] ?? 0,
