...
//            throw new ReservationOffException();
        ...
        $this->pickUpSlot = collect();
        $slotId = $this->payload['slot_id'] ?? 0;

        /*<---- if allow future time reservation is off then show all time slot else show now only --->*/
        ...
        /*<--- add now slot into slots ---->*/
        if ($this->date == Carbon::now()->format('Y-m-d')) {
            $cloneSlot = cloneSlot($this->store, $this->meal);
            $this->pickUpSlot = $this->pickUpSlot->prepend($cloneSlot);
        }
    }
